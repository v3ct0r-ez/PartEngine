...
| Attachments / datasheet OCR | ✅ | Sprint 7: upload/list/download/delete (local FS storage), PDF/txt text extraction (pdf-parse) → searchable ocrText, field suggestion from datasheet; UI panel in editor. **Image OCR**: images are OCR'd in-process with tesseract.js (lazy-loaded; bundled wasm core, language data fetched once and cached) in the background after upload, filling ocrText → searchable + suggestions. Scanned image-only PDFs still need rasterization (future) |
