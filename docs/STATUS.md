...
| Attachments / datasheet OCR | ✅ | Sprint 7: upload/list/download/delete (local FS storage), PDF/txt text extraction (pdf-parse) → searchable ocrText, field suggestion from datasheet; UI panel in editor. **Image & scanned-PDF OCR**: images are OCR'd in-process with tesseract.js (lazy-loaded; bundled wasm core, language data fetched once and cached); scanned image-only PDFs are rasterized with pdf.js + a Node canvas (first 5 pages) then OCR'd. Runs in the background after upload, filling ocrText → searchable + suggestions. An `ocrStatus` (NONE/PENDING/DONE/FAILED) drives an OK/Error/in-progress badge in the editor's attachments panel |
